"""
Overview:
This module handles all logic for recommending openings based on peer performance.
It filters games by peer list and color, computes performance statistics for each
opening, and selects the top openings by a weighted score.

Pipeline Usage:
1. After identifying a set of stylistic peers (`peer_list`) and loading a DataFrame
...
get_peer_games(elite_df, peer_list) -> pd.DataFrame  
    Returns the subset of `elite_df` where either White or Black is in `peer_list`.
...
compute_opening_stats(
    ...
    opening (ECO code + name) to compute:
      ...
      • score_pct     — normalized performance: (wins + 0.5 × draws) / games_played  
      • weight        — `score_pct` weighted by sample size: score_pct × log10(games_played + 1)  
...
def compute_opening_stats(
    ...
    return stats.sort_values(['weight','games_played'], ascending=False).reset_index(drop=True)
...
def recommend_openings(
    ...
    white_recs = white_stats.head(top_n)[['eco','opening','games_played','score_pct']]\
                  .values.tolist()
    black_recs = black_stats.head(top_n)[['eco','opening','games_played','score_pct']]\
                  .values.tolist()
