...
"""
Overview:
Centralizes data collection and parsing routines for the chess opening recommender.
...
pgn_to_games_df(pgn_text: str) -> pd.DataFrame
    - Parses a multi-game pgn string into a dataframe with columns:
      white, black, result, eco, opening, utc_date, utc_time, time_control, moves (list), evals (list)
...
parse_elite_pgn_fast(pgn_path: Path, n_games: int = 500) -> pd.DataFrame
    - Streams the first `n_games` from the elite pgn file, parses headers+moves+evals, returns as dataframe
...
#stream the first n_games from the elite pgn file, parse headers+moves+evals, return as dataframe
