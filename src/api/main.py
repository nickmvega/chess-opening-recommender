...
from recommender.feature_engineering import extract_style_features, summarize_player_features
from recommender.clustering import cluster_styles, find_style_neighbors
from recommender.opening_recommender import compute_opening_stats, recommend_openings
...
def recommend_all(
    username: str,
    time_control: Optional[str] = Query(None, description="bullet, blitz, rapid, classical"),
):
    user = username.strip()
    if not user:
    ...
    if time_control:
        games_df = games_df[games_df["time_control"].str.contains(time_control, na=False)]
        if games_df.empty:
    ...
    peer_games = elite_games_df[
        elite_games_df["white"].isin(peer_list) | elite_games_df["black"].isin(peer_list)
    ]
    if time_control:
        peer_games = peer_games[peer_games["time_control"].str.contains(time_control, na=False)]
